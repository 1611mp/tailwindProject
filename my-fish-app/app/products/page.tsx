    ...
    <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
      {products.map((product: Product, idx: number) => {
        ...
        return (
          <li
            key={id}
            className="bg-white rounded-2xl shadow hover:shadow-lg transition overflow-hidden group flex flex-col"
          >
            {/* Product Image */}
            {product.image && (
            ...
            <div className="p-5 flex flex-col flex-1">
              ...
              <p className="text-gray-600 mb-3 flex-1">{product.description}</p>

              {/* Buttons */}
              <div className="mt-4 flex gap-3">
                <button
                  className={`flex-1 rounded-md bg-indigo-600 text-white py-2 hover:bg-indigo-700 transition relative ${addingId === id ? 'opacity-60 cursor-wait' : ''}`}
                ...
                >
                  {addingId === id ? (
                    ...
                    <span className="flex items-center justify-center text-green-200">Added!</span>
                  ) : (
                    'Add to Cart'
                  )}
                </button>
                <button
                  className="flex-1 rounded-md border border-indigo-600 text-indigo-600 py-2 hover:bg-indigo-50 transition"
                >
                  ₹{product.price}
                </button>
              </div>
